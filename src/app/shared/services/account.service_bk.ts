...
export class AccountService {
    ...
    //     return this.httpClient.post<User>(`${environment.apiUrl}yha/login`, { username, password })
    //         .pipe(map(user => {
    ...
    login(email: string, password: string) {
        ...
        // console.log("api url:",this.apiUrlYha + '/login')
        return this.httpClient.post(this.apiUrlYha + '/login',{},{ params: params })
        /**
         * Login session is being handled on server side with Flask backend
         */
        // .pipe(map(user => {
        //     // store user details and jwt token in local storage to keep user logged in between page refreshes
        //     console.log("user: ",user)
